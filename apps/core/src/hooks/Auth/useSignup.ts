...
const useSignup = () => {
  ...
  const { mutate: reqAuthCode, isPending: reqLoading } = useReqAuthCode();
  const { mutate: sendAuthCode, isPending: sendLoading } = useSendAuthCode();
